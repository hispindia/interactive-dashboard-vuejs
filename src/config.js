const yll_id = "KHRNfvdT9dA;qZvcjbS6Vaq;XkvbmSQNWS0;PP5wOeAYiO3;ttdmF6w8y5T;ItofpP0Eo2H;Ft4sC50UywO;lkZH71b2qqI;oHxYHpmJFTI;oE12jKw1cDn;jUOkNisSGtL;YAZdpC2fdlp;qSGAN58i4Y4;ycnYaICFBo5;tAlC7ioAkP3;YV6xDaZiFOA;Gzmf8Brlebv;te717AMOBPU;Y90XWSZUXil;u1vQIT0KDmq;OnFNzSJmpDd;b8bF2H1nlKM;rbvkGJD9mNb;HoAv6hDtfx9;nac95mZ6R3J;MWl8SCPJLAE;Q3gKBv7Q76G;ZenTjUJdMit;WADeiOG2GZ3;K3vVuinN0sf;q1y9nxTmGCB;Lo9m9x0Noo3;zTXCQFLMd07;f54afn66e00;TqAnqg4TGIi;bo2ysWfPPNp;kJb0YxZ3fC7;hqkcNhlZa3l;LiDCa42AKC1;wiGw1nW0bh8;DxDvbzbqX8U;pEmQPvuMsCd;WlkD7uIrJS1";

const yld_id = "FQ0sR0qweYg;uP0MRaB6mQw;CPSynCWa3F3;fNFwiFlIZJj;wu9HKrW7NCs;kLxYWOjvia5;YX1PqtOLQPB;ppbHUdxuwKI;avpol7zKA8v;LwsahLjmCuA;UVzGycrQd5s;F1aGfaHeIH0;pIZiJ1IGrM2;i7YMxsvWjRP;JgBf1i8mTl7;cBGfQMmf14S;nPWTo6eIfXG;oF5uiIRpvgf;NFRkMwepFNF;xqH3j6yJj2Q;QrYqHe9G8oO;pLvYReT9ZWZ;whnTaAL6FF4;WK5CwUWJeuz;W5vIBAz8UrL;wmMIOwY7zOQ;FWpn1mpMVAH;pjiyF1CgTdq;Hqgx3Gr87Qz;CZuCLz8ZkGh;qEs4931bbzH;TpXDC1IENWI;CQLiH0SNXxv;HmQgoDfATlx;sZogSMHVw8o;d0HG4Jex0Sv;lG7TfAsy7CB;hgPrC3bMBnS;aJgBZ0baQDM;aOrC0GcrCVT;pzNN8VLUYa6;gnbdk2vc0uW";

const daly_id = "gxhQpCbFB3Q;xtKp6I7IW26;yIyNAOM1u9Z;cmCvW0dqYIU;Zy9AZUbeJdu;lXK2YGC8Wnm;ipm4P5vdH3O;OcJVS6d9JYV;YcSgwuKbLVM;YxmshxPQfFT;JsYEGdPYKXV;hz3xiC028bl;nLSIZCh2DSI;A5iw1YRUUoG;vmoQPXY2pVk;FtDFPXEUzVS;MgPog0EwzQi;FyUxNHcmGQb;pf9fXcVMch1;OaP6C0RHURO;IxZrLmK9onl;GaYrrMmz1KB;uqUmzU6bjbF;ed41dKPKdhx;E4ThmXrKE83;p1eQckB6I12;Ken3MHrMEUR;g57Aq6ENncW;sXDbduD9Sei;mHrDw0dGvc9;x0bg1YZkRL7;cbvHNRyXPLX;WK6PFCzY5ey;VuEOD18STGS;x6OFcxMCTVw;s3E5MECaJga;xJTNZfIyiF7;dFv9IGNFyq5;OaeHix9duCn;CCPvTbThNQC;QRUZa2NHY35;sBzkGMgeqz5;wPKc8aPWoI2";

const death_id = "G88aHJhkSyR;eTAKBjIkwaz;rR11LCFjkoT;EI8xZCX8bCA;GyJw0WaL92N;MpZbNyRQPSP;VUm3woORMNB;JF3BEE0kYoJ;QKXrL8lZetJ;T11thVOCxbC;eHvuTO8pQiF;kh9RIuLy8tg;rjqa3BQE649;X8fpBqisVaW;EX9IN3ypvdY;ZZIrvfpFn1S;XFL2f8OlAaM;jB5NguyWFz6;sVmArtwPXJX;QZKKmntYWnK;hGi1Pq8H3ix;RwB0sPs9WL1;WUzmQKiWNOt;KjyPR5oK8KF;KRNbMneERb7;P1bxpIcNaJe;IwliRMQtcwI;N8pnKAkSuS9;yKYmGtoLvuO;dhFdGBrCOL0;MszRYeCv7tb;EHDA6YRi77G;WclEXSqNL8q;HoTW2c63jQZ;wxcHhR9gF2v;hhZ2dn7aLRx;uDP7MrmByBK;NKpDx4DLbCX;ykL5JPlWrC3;trGfGTQGj5J;pDz4OePMon1;vQLeSv0U8zm;v3x2vzgTp7a;DpboXWZn1kz;vGvSM6QeW8v;Uq5cNdR3bl8;";


const age_id = "kTkbVocs2qt";
const age_0_4_id = "LJglh3dKkBF";
const age_5_14_id = "Y80cGPh9cA9";
const age_15_29_id = "ppIxci61gTu";
const age_30_49_id = "yvdQkzHuq2E";
const age_50_59_id = "fohLv27MtjN";
const age_60_69_id = "FPQcIz7kBvD";
const age_70_79_id = "fuZTseIa9nS";
const age_above80_id = "rHmIMu8bVmy";
const age_categories = ["0-4", "5-14", "15-29", "30-49", "50-59", "60-69", "70-79", "80 and Above"];

const age_main_var = "LJglh3dKkBF;Y80cGPh9cA9;ppIxci61gTu;yvdQkzHuq2E;fohLv27MtjN;FPQcIz7kBvD;fuZTseIa9nS;rHmIMu8bVmy"

const all_ages_main_id = "WEM8mVPOhM7";
const all_ages_id = "KRxXmwGhNrr";

const gender_id = "Qw8MkDKTvPV";
const gender_male_id = "lEUL3JB7ZCA";
const gender_female_id = "RXScRq7ZbYm";
const gender_categories = ["Male", "Female", "Both"];
const gender_categories_forpie = ["Male", "Female"];

const gender_main_var = "lEUL3JB7ZCA;RXScRq7ZbYm";

const site_id = "hMtyUrlNCdp";
const site_urban_id = "GkHE9YVFkpE";
const site_rural_id = "Vl6ZLaCt2mW";
const site_categories = ["Urban", "Rural", "Both"];
const site_categories_forpie = ["Urban", "Rural"];

const site_main_var = "GkHE9YVFkpE;Vl6ZLaCt2mW";

const diseases_obj_yll = [{
    'rbvkGJD9mNb': {
        name: 'Malaria - YLL',
        data: [],
        val: 0,
        color: "",
        index: 0,
        group: 0
    },
    'wiGw1nW0bh8': {
        name: 'Sexually-transmitted infections incl. HIV/AIDS - YLL',
        data: [],
        val: 0,
        color: "",
        index: 1,
        group: 0
    },
    'oHxYHpmJFTI': {
        name: 'Diarrhoeal diseases - YLL',
        data: [],
        val: 0,
        color: "",
        index: 2,
        group: 0
    },
    'hqkcNhlZa3l': {
        name: 'Selected vaccine preventable diseases - YLL',
        data: [],
        val: 0,
        color: "",
        index: 3,
        group: 0
    },
    'nac95mZ6R3J': {
        name: 'Meningitis/encephalitis - YLL',
        data: [],
        val: 0,
        color: "",
        index: 4,
        group: 0
    },
    'YV6xDaZiFOA': {
        name: 'Hepatitis - YLL',
        data: [],
        val: 0,
        color: "",
        index: 5,
        group: 0
    },
    'TqAnqg4TGIi': {
        name: 'Respiratory infections - YLL',
        data: [],
        val: 0,
        color: "",
        index: 6,
        group: 0
    },
    'Lo9m9x0Noo3': {
        name: 'Other infectious and parasitic diseases - YLL',
        data: [],
        val: 0,
        color: "",
        index: 7,
        group: 0
    },
    'HoAv6hDtfx9': {
        name: 'Maternal conditions - YLL',
        data: [],
        val: 0,
        color: "",
        index: 8,
        group: 0
    },
    'f54afn66e00': {
        name: 'Perinatal conditions - YLL',
        data: [],
        val: 0,
        color: "",
        index: 9,
        group: 0
    },
    'ZenTjUJdMit': {
        name: 'Nutritional deficiencies - YLL',
        data: [],
        val: 0,
        color: "",
        index: 10,
        group: 0
    },
    'qSGAN58i4Y4': {
        name: 'Fever of unknown origin - YLL',
        data: [],
        val: 0,
        color: "",
        index: 11,
        group: 0
    },
    'WlkD7uIrJS1': {
        name: 'Tuberculosis YLL',
        data: [],
        val: 0,
        color: "",
        index: 12,
        group: 0
    },
    'jUOkNisSGtL': {
        name: 'Epilepsy - YLL',
        data: [],
        val: 0,
        color: "",
        index: 13,
        group: 1
    },
    'pEmQPvuMsCd': {
        name: 'Stomach cancer - YLL',
        data: [],
        val: 0,
        color: "",
        index: 14,
        group: 1
    },
    'b8bF2H1nlKM': {
        name: 'Lung & airway cancers - YLL',
        data: [],
        val: 0,
        color: "",
        index: 15,
        group: 1
    },
    'WADeiOG2GZ3': {
        name: 'Oral and upper aerodigestive cancers - YLL',
        data: [],
        val: 0,
        color: "",
        index: 16,
        group: 1
    },
    'u1vQIT0KDmq': {
        name: 'Liver cancer - YLL',
        data: [],
        val: 0,
        color: "",
        index: 17,
        group: 1
    },
    'K3vVuinN0sf': {
        name: 'Other digestive cancers - YLL',
        data: [],
        val: 0,
        color: "",
        index: 18,
        group: 1
    },
    'XkvbmSQNWS0': {
        name: 'Breast cancer - YLL',
        data: [],
        val: 0,
        color: "",
        index: 19,
        group: 1
    },
    'ttdmF6w8y5T': {
        name: 'Cervix & uterus cancers - YLL',
        data: [],
        val: 0,
        color: "",
        index: 20,
        group: 1
    },
    'Y90XWSZUXil': {
        name: 'Leukemia,lymphoma and oth. hemotopoietic malignancies - YLL',
        data: [],
        val: 0,
        color: "",
        index: 21,
        group: 1
    },
    'KHRNfvdT9dA': {
        name: 'All other cancers - YLL',
        data: [],
        val: 0,
        color: "",
        index: 22,
        group: 1
    },
    'lkZH71b2qqI': {
        name: 'Diabetes, endocrine and immune disorders - YLL',
        data: [],
        val: 0,
        color: "",
        index: 23,
        group: 1
    },
    'zTXCQFLMd07': {
        name: 'Other neuropsychiatric conditions (excl. epilepsy) - YLL',
        data: [],
        val: 0,
        color: "",
        index: 24,
        group: 1
    },
    'DxDvbzbqX8U': {
        name: 'Skin, sense organs and Oral conditions - YLL',
        data: [],
        val: 0,
        color: "",
        index: 25,
        group: 1
    },
    'MWl8SCPJLAE': {
        name: 'Musculoskeletal disorders - YLL',
        data: [],
        val: 0,
        color: "",
        index: 26,
        group: 1
    },
    'bo2ysWfPPNp': {
        name: 'Rheumatic heart disease - YLL',
        data: [],
        val: 0,
        color: "",
        index: 27,
        group: 1
    },
    'PP5wOeAYiO3': {
        name: 'Cerebrovascular diseases - YLL',
        data: [],
        val: 0,
        color: "",
        index: 28,
        group: 1
    },
    'fEdSIl29SXB': {
        name: 'Heart diseases - YLL',
        data: [],
        val: 0,
        color: "",
        index: 29,
        group: 1
    },
    'ItofpP0Eo2H': {
        name: 'Chronic respiratory diseases - YLL',
        data: [],
        val: 0,
        color: "",
        index: 30,
        group: 1
    },
    'ycnYaICFBo5': {
        name: 'Gastro-oesophageal diseases - YLL',
        data: [],
        val: 0,
        color: "",
        index: 31,
        group: 1
    },
    'OnFNzSJmpDd': {
        name: 'Liver diseases - YLL',
        data: [],
        val: 0,
        color: "",
        index: 32,
        group: 1
    },
    'q1y9nxTmGCB': {
        name: 'Other digestive diseases - YLL',
        data: [],
        val: 0,
        color: "",
        index: 33,
        group: 1
    },
    'Q3gKBv7Q76G': {
        name: 'Nephritis and nephrosis - YLL',
        data: [],
        val: 0,
        color: "",
        index: 34,
        group: 1
    },
    'tAlC7ioAkP3': {
        name: 'Geneto-urintary diseases - YLL',
        data: [],
        val: 0,
        color: "",
        index: 35,
        group: 1
    },
    'Ft4sC50UywO': {
        name: 'Congenital anomalies - YLL',
        data: [],
        val: 0,
        color: "",
        index: 36,
        group: 1
    },
    'oE12jKw1cDn': {
        name: 'Drowning - YLL',
        data: [],
        val: 0,
        color: "",
        index: 37,
        group: 2
    },
    'LiDCa42AKC1': {
        name: 'Self-inflicted injuries (suicide) - YLL',
        data: [],
        val: 0,
        color: "",
        index: 38,
        group: 2
    },
    'qZvcjbS6Vaq': {
        name: 'All other injuries - YLL',
        data: [],
        val: 0,
        color: "",
        index: 39,
        group: 2
    },
    'Tb3Xvv9wUA4': {
        name: 'Venomous deaths - YLL',
        data: [],
        val: 0,
        color: "",
        index: 40,
        group: 2
    },
    'te717AMOBPU': {
        name: 'Interpersonal violence - YLL',
        data: [],
        val: 0,
        color: "",
        index: 41,
        group: 2
    },
    'kJb0YxZ3fC7': {
        name: 'Road traffic accidents - YLL',
        data: [],
        val: 0,
        color: "",
        index: 42,
        group: 2
    },
    'YAZdpC2fdlp': {
        name: 'Falls - YLL',
        data: [],
        val: 0,
        color: "",
        index: 44,
        group: 2
    },
    'Gzmf8Brlebv': {
        name: 'Ill-defined or cause unknown - YLL',
        data: [],
        val: 0,
        color: "",
        index: 45,
        group: 3
    },
}];
const ages_obj = [{
    'LJglh3dKkBF': {
        name: '0-4 Years',
        data: [],
        val: 0,
        color: "",
        index:0,
        group: 1
    },
    'Y80cGPh9cA9': {
        name: '5-14 Years',
        data: [],
        val: 0,
        color: "",
        index: 1,
        group: 0
    },
    'ppIxci61gTu': {
        name: '15-29 Years',
        data: [],
        val: 0,
        color: "",
        index: 2,
        group: 0
    },
    'yvdQkzHuq2E': {
        name: '30-49 Years',
        data: [],
        val: 0,
        color: "",
        index: 3,
        group: 0
    },
    'fohLv27MtjN': {
        name: '50-59 Years',
        data: [],
        val: 0,
        color: "",
        index: 4,
        group: 0
    },
    'FPQcIz7kBvD': {
        name: '60-69 Years',
        data: [],
        val: 0,
        color: "",
        index: 5,
        group: 0
    },
    'fuZTseIa9nS': {
        name: '70-79 Years',
        data: [],
        val: 0,
        color: "",
        index: 6,
        group: 0
    },
    'rHmIMu8bVmy': {
        name: '80 Years & Above',
        data: [],
        val: 0,
        color: "",
        index: 7,
        group: 0
    },
}];
const diseases_group = [{
    'cbEjdFMYLh0': {
        name: "Communicable, maternal, perinatal and nutritional conditions",
        data: [],
        val: 0,
        color: "",
        index: 0,
        group: 2
    },
    'S4yHg2jEMXI': {
        name: "Ill-defined or cause unknown",
        data: [],
        val: 0,
        color: "",
        index: 1,
        group: 1
    },
    'MGK3XaVs7MT': {
        name: "Injuries",
        data: [],
        val: 0,
        color: "",
        index: 2,
        group: 0
    },
    'M98SAuHB7LJ': {
        name: "Noncommunicable diseases",
        data: [],
        val: 0,
        color: "",
        index: 3,
        group: 1
    }
}];
const diseases_group_1 = [{
    'KjyPR5oK8KF': {
        name: "Maternal conditions (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 0,
        group: 2
    },
    'LdRpJ0ydVJQ': {
        name: "Sexually-transmitted infections incl. HIV/AIDS (median)",
        data: [],
        val: 0,
        color: "",
        index: 1,
        group: 1
    },
    'hhZ2dn7aLRx': {
        name: "Respiratory infections (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 2,
        group: 0
    },
    'rjqa3BQE649': {
        name: "Fever of unknown origin (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 3,
        group: 1
    },
    'EX9IN3ypvdY': {
        name: "Hepatitis (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 4,
        group: 1
    },
    'vGvSM6QeW8v': {
        name: "Tuberculosis (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 5,
        group: 1
    },
    'wxcHhR9gF2v': {
        name: "Perinatal conditions (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 6,
        group: 1
    },
    'WUzmQKiWNOt': {
        name: "Malaria (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 7,
        group: 1
    },
    'KRNbMneERb7': {
        name: "Meningitis/encephalitis (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 8,
        group: 1
    },
    'pDz4OePMon1': {
        name: "Sexually-transmitted infections incl. HIV/AIDS (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 9,
        group: 1
    },
    'HoTW2c63jQZ': {
        name: "Other infectious and parasitic diseases (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 10,
        group: 1
    },
    'ykL5JPlWrC3': {
        name: "Selected vaccine preventable diseases (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 11,
        group: 1
    },
    'N8pnKAkSuS9': {
        name: "Nutritional deficiencies (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 13,
        group: 1
    },
    'QKXrL8lZetJ': {
        name: "Diarrhoeal diseases (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 12,
        group: 1
    }
}];

const diseases_group_4 = [{
    'X8fpBqisVaW': {
        name: "Gastro-oesophageal diseases (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 0,
        group: 2
    },
    'QZKKmntYWnK': {
        name: "Liver and alcohol related diseases (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 1,
        group: 1
    },
    'dhFdGBrCOL0': {
        name: "Other Genitorurintary diseases (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 2,
        group: 0
    },
    'uDP7MrmByBK': {
        name: "Rheumatic heart disease (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 3,
        group: 1
    },
    'P1bxpIcNaJe': {
        name: "Musculoskeletal disorders (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 4,
        group: 1
    },
    'rR11LCFjkoT': {
        name: "Breast cancer (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 5,
        group: 1
    },
    'sVmArtwPXJX': {
        name: "Leukemia,lymphoma and other hemotopoietic malignancies (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 6,
        group: 1
    },
    'MpZbNyRQPSP': {
        name: "Chronic respiratory diseases (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 7,
        group: 1
    },
    'vQLeSv0U8zm': {
        name: "Skin, Sense Organ, Oral conditions (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 8,
        group: 1
    }
}];

const diseases_group_3 = [{
    'Uq5cNdR3bl8': {
        name: "Venomous deaths (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 0,
        group: 2
    },
    'kh9RIuLy8tg': {
        name: "Falls (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 1,
        group: 1
    },
    'trGfGTQGj5J': {
        name: "Self-inflicted injuries (suicide) (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 2,
        group: 0
    },
    'T11thVOCxbC': {
        name: "Drowning (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 3,
        group: 1
    },
    'XFL2f8OlAaM': {
        name: "Interpersonal violence (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 4,
        group: 1
    },
    'eTAKBjIkwaz': {
        name: "All other injuries (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 5,
        group: 1
    },
    'NKpDx4DLbCX': {
        name: "Road Traffic accidents (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 6,
        group: 1
    }
}];

const diseases_group_2 = [{
    'ZZIrvfpFn1S': {
        name: "Ill-defined or cause unknown (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 0,
        group: 1
    }
}];
const diseases_obj_deaths = [{
    'VUm3woORMNB': {
        name: "Congenital anomalies (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 36,
        group: 2
    },
    'Ij1ozoYgNiV': {
        name: "Heart diseases (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 13,
        group: 1
    },
    'KjyPR5oK8KF': {
        name: "Maternal conditions (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 0,
        group: 0
    },
    'yKYmGtoLvuO': {
        name: "Oral and Upper aerodigestive cancers (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 14,
        group: 1
    },
    'vQLeSv0U8zm': {
        name: "Skin, Sense Organ, Oral conditions (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 15,
        group: 1
    },
    'dhFdGBrCOL0': {
        name: "Other Genitorurintary diseases (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 45,
        group: 3
    },
    'EI8xZCX8bCA': {
        name: "Cerebrovascular disease (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 16,
        group: 1
    },
    'HoTW2c63jQZ': {
        name: "Other infectious and parasitic diseases (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 1,
        group: 0
    },
    'WUzmQKiWNOt': {
        name: "Malaria (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 2,
        group: 0
    },
    'X8fpBqisVaW': {
        name: "Gastro-oesophageal diseases (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 17,
        group: 1
    },
    'MpZbNyRQPSP': {
        name: "Chronic respiratory diseases (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 18,
        group: 1
    },
    'ZZIrvfpFn1S': {
        name: "Ill-defined or cause unknown (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 46,
        group: 3
    },
    'QZKKmntYWnK': {
        name: "Liver and alcohol related diseases (deaths)",
        data: [],
        val: 0,
        group: 1,
        index: 19
    },
    'uDP7MrmByBK': {
        name: "Rheumatic heart disease (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 20,
        group: 1
    },
    'hGi1Pq8H3ix': {
        name: "Liver cancer (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 21,
        group: 1
    },
    'IwliRMQtcwI': {
        name: "Nephritis and Nephrosis (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 22,
        group: 1
    },
    'v3x2vzgTp7a': {
        name: "Snakebite and other venemous (deaths)",
        data: [],
        val: 0,
        group: 2,
        index: 37
    },
    'KRNbMneERb7': {
        name: "Meningitis/encephalitis (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 3,
        group: 0
    },
    'G88aHJhkSyR': {
        name: "All other cancers (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 23,
        group: 1
    },
    'NKpDx4DLbCX': {
        name: "Road Traffic accidents (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 38,
        group: 2
    },
    'jB5NguyWFz6': {
        name: "Ischemic Heart disease (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 24,
        group: 1
    },
    'MszRYeCv7tb': {
        name: "Other Neuropsychiatric conditions (deaths)",
        data: [],
        val: 0,
        group: 1,
        index: 25
    },
    'P1bxpIcNaJe': {
        name: "Musculoskeletal disorders (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 26,
        group: 1
    },
    'T11thVOCxbC': {
        name: "Drowning (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 39,
        group: 2
    },
    'GyJw0WaL92N': {
        name: "Cervix & uterus cancers (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 27,
        group: 1
    },
    'kh9RIuLy8tg': {
        name: "Falls (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 40,
        group: 2
    },
    'eHvuTO8pQiF': {
        name: "Epilepsy (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 28,
        group: 1
    },
    'EHDA6YRi77G': {
        name: "Other digestive cancer (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 29,
        group: 1
    },
    'rjqa3BQE649': {
        name: "Fever of unknown origin (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 4,
        group: 0
    },
    'EX9IN3ypvdY': {
        name: "Hepatitis (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 5,
        group: 0
    },
    'JF3BEE0kYoJ': {
        name: "Diabetes, Endocrine and immune disorders (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 30,
        group: 1
    },
    'pDz4OePMon1': {
        name: "Sexually-transmitted infections incl. HIV/AIDS (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 6,
        group: 0
    },
    'Uq5cNdR3bl8': {
        name: "Venomous deaths (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 41,
        group: 2
    },
    'trGfGTQGj5J': {
        name: "Self-inflicted injuries (suicide) (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 42,
        group: 2
    },
    'sVmArtwPXJX': {
        name: "Leukemia,lymphoma and other hemotopoietic malignancies (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 31,
        group: 1
    },
    'XFL2f8OlAaM': {
        name: "Interpersonal violence (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 43,
        group: 2
    },
    'QKXrL8lZetJ': {
        name: "Diarrhoeal diseases (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 7,
        group: 0
    },
    'DpboXWZn1kz': {
        name: "Stomach cancer (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 32,
        group: 1
    },
    'hhZ2dn7aLRx': {
        name: "Respiratory infections (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 8,
        group: 0
    },
    'N8pnKAkSuS9': {
        name: "Nutritional deficiencies (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 9,
        group: 0
    },
    'WclEXSqNL8q': {
        name: "Other digestive diseases (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 33,
        group: 1
    },
    'wxcHhR9gF2v': {
        name: "Perinatal conditions (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 10,
        group: 0
    },
    'eTAKBjIkwaz': {
        name: "All other injuries (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 44,
        group: 2
    },
    'vGvSM6QeW8v': {
        name: "Tuberculosis (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 11,
        group: 0
    },
    'RwB0sPs9WL1': {
        name: "Lung & airway cancers (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 34,
        group: 1
    },
    'ykL5JPlWrC3': {
        name: "Selected vaccine preventable diseases (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 12,
        group: 0
    },
    'rR11LCFjkoT': {
        name: "Breast cancer (deaths)",
        data: [],
        val: 0,
        color: "",
        index: 35,
        group: 1
    }
}];

const diseases_obj_yld = [{
    'cBGfQMmf14S': {
        name: "Hepatitis - YLD",
        data: [],
        val: 0,
        color: "",
        index: 0,
        group: 0
    },
    'd0HG4Jex0Sv': {
        name: "Rheumatic heart disease - YLD",
        data: [],
        val: 0,
        color: "",
        index: 13,
        group: 1
    },
    'WK5CwUWJeuz': {
        name: "Maternal conditions - YLD",
        data: [],
        val: 0,
        color: "",
        index: 1,
        group: 0
    },
    'pzNN8VLUYa6': {
        name: "Skin, sense organs and Oral conditions - YLD",
        data: [],
        val: 0,
        color: "",
        index: 14,
        group: 1
    },
    'xqH3j6yJj2Q': {
        name: "Liver cancer - YLD",
        data: [],
        val: 0,
        color: "",
        index: 15,
        group: 1
    },
    'aOrC0GcrCVT': {
        name: "Sexually-transmitted infections incl. HIV/AIDS - YLD",
        data: [],
        val: 0,
        color: "",
        index: 2,
        group: 0
    },
    'oF5uiIRpvgf': {
        name: "Interpersonal violence - YLD",
        data: [],
        val: 0,
        color: "",
        index: 36,
        group: 2
    },
    'oPcKTUUPxMv': {
        name: "Venomous deaths - YLD",
        data: [],
        val: 0,
        color: "",
        index: 37,
        group: 2
    },
    'kLxYWOjvia5': {
        name: "Chronic respiratory diseases - YLD",
        data: [],
        val: 0,
        color: "",
        index: 16,
        group: 1
    },
    'UVzGycrQd5s': {
        name: "Epilepsy - YLD",
        data: [],
        val: 0,
        color: "",
        index: 17,
        group: 1
    },
    'qEs4931bbzH': {
        name: "Other digestive diseases - YLD",
        data: [],
        val: 0,
        color: "",
        index: 18,
        group: 1
    },
    'wmMIOwY7zOQ': {
        name: "Musculoskeletal disorders - YLD",
        data: [],
        val: 0,
        color: "",
        index: 19,
        group: 1
    },
    'wu9HKrW7NCs': {
        name: "Cervix & uterus cancers - YLD",
        data: [],
        val: 0,
        color: "",
        index: 20,
        group: 1
    },
    'YX1PqtOLQPB': {
        name: "Congenital anomalies - YLD",
        data: [],
        val: 0,
        color: "",
        index: 38,
        group: 2
    },
    'HmQgoDfATlx': {
        name: "Perinatal conditions - YLD",
        data: [],
        val: 0,
        color: "",
        index: 3,
        group: 0
    },
    'FWpn1mpMVAH': {
        name: "Nephritis and nephrosis - YLD",
        data: [],
        val: 0,
        color: "",
        index: 21,
        group: 1
    },
    'F1aGfaHeIH0': {
        name: "Falls - YLD",
        data: [],
        val: 0,
        color: "",
        index: 39,
        group: 2
    },
    'QrYqHe9G8oO': {
        name: "Liver diseases - YLD",
        data: [],
        val: 0,
        color: "",
        index: 22,
        group: 1
    },
    'nPWTo6eIfXG': {
        name: "Ill-defined or cause unknown - YLD",
        data: [],
        val: 0,
        color: "",
        index: 44,
        group: 3
    },
    'eGOdxT55PxY': {
        name: "Heart diseases - YLD",
        data: [],
        val: 0,
        color: "",
        index: 23,
        group: 1
    },
    'sZogSMHVw8o': {
        name: "Respiratory infections - YLD",
        data: [],
        val: 0,
        color: "",
        index: 4,
        group: 0
    },
    'CZuCLz8ZkGh': {
        name: "Other digestive cancers - YLD",
        data: [],
        val: 0,
        color: "",
        index: 24,
        group: 1
    },
    'pjiyF1CgTdq': {
        name: "Nutritional deficiencies - YLD",
        data: [],
        val: 0,
        color: "",
        index: 5,
        group: 0
    },
    'zd2aCwJGvOH': {
        name: "Tuberculosis YLD",
        data: [],
        val: 0,
        color: "",
        index: 6,
        group: 0
    },
    'avpol7zKA8v': {
        name: "Diarrhoeal diseases - YLD",
        data: [],
        val: 0,
        color: "",
        index: 7,
        group: 0
    },
    'aJgBZ0baQDM': {
        name: "Self-inflicted injuries (suicide) - YLD",
        data: [],
        val: 0,
        color: "",
        index: 40,
        group: 2
    },
    'CPSynCWa3F3': {
        name: "Breast cancer - YLD",
        data: [],
        val: 0,
        color: "",
        index: 25,
        group: 1
    },
    'pIZiJ1IGrM2': {
        name: "Fever of unknown origin - YLD",
        data: [],
        val: 0,
        color: "",
        index: 8,
        group: 0
    },
    'Hqgx3Gr87Qz': {
        name: "Oral and upper aerodigestive cancers - YLD",
        data: [],
        val: 0,
        color: "",
        index: 26,
        group: 1
    },
    'LwsahLjmCuA': {
        name: "Drowning - YLD",
        data: [],
        val: 0,
        color: "",
        index: 41,
        group: 2
    },
    'FQ0sR0qweYg': {
        name: "All other cancers - YLD",
        data: [],
        val: 0,
        color: "",
        index: 27,
        group: 1
    },
    'NFRkMwepFNF': {
        name: "Leukemia,lymphoma and oth. hemotopoietic malignancies - YLD",
        data: [],
        val: 0,
        color: "",
        index: 28,
        group: 1
    },
    'JgBf1i8mTl7': {
        name: "Geneto-urintary diseases - YLD",
        data: [],
        val: 0,
        color: "",
        index: 29,
        group: 1
    },
    'ppbHUdxuwKI': {
        name: "Diabetes, endocrine and immune disorders - YLD",
        data: [],
        val: 0,
        color: "",
        index: 30,
        group: 1
    },
    'i7YMxsvWjRP': {
        name: "Gastro-oesophageal diseases - YLD",
        data: [],
        val: 0,
        color: "",
        index: 31,
        group: 1
    },
    'gnbdk2vc0uW': {
        name: "Stomach cancer - YLD",
        data: [],
        val: 0,
        color: "",
        index: 32,
        group: 1
    },
    'pLvYReT9ZWZ': {
        name: "Lung & airway cancers - YLD",
        data: [],
        val: 0,
        color: "",
        index: 33,
        group: 1
    },
    'hgPrC3bMBnS': {
        name: "Selected vaccine preventable diseases - YLD",
        data: [],
        val: 0,
        color: "",
        index: 9,
        group: 0
    },
    'W5vIBAz8UrL': {
        name: "Meningitis/encephalitis - YLD",
        data: [],
        val: 0,
        color: "",
        index: 10,
        group: 0
    },
    'TpXDC1IENWI': {
        name: "Other infectious and parasitic diseases - YLD",
        data: [],
        val: 0,
        color: "",
        index: 11,
        group: 0
    },
    'CQLiH0SNXxv': {
        name: "Other neuropsychiatric conditions (excl. epilepsy) - YLD",
        data: [],
        val: 0,
        group: 1,
        index: 34
    },
    'whnTaAL6FF4': {
        name: "Malaria - YLD",
        data: [],
        val: 0,
        color: "",
        index: 12,
        group: 0
    },
    'fNFwiFlIZJj': {
        name: "Cerebrovascular diseases - YLD",
        data: [],
        val: 0,
        color: "",
        index: 35,
        group: 1
    },
    'uP0MRaB6mQw': {
        name: "All other injuries - YLD",
        data: [],
        val: 0,
        color: "",
        index: 42,
        group: 2
    },
    'lG7TfAsy7CB': {
        name: "Road traffic accidents - YLD",
        data: [],
        val: 0,
        color: "",
        index: 43,
        group: 2
    }
}];

const diseases_obj_daly = [{
    "OcJVS6d9JYV": {
        name: "Diabetes, endocrine and immune disorders - DALY",
        data: [],
        val: 0,
        color: "",
        index: 13,
        group: 1
    },
    "JsYEGdPYKXV": {
        name: "Epilepsy - DALY",
        data: [],
        val: 0,
        color: "",
        index: 14,
        group: 1
    },
    "IxZrLmK9onl": {
        name: "Liver diseases - DALY",
        data: [],
        val: 0,
        color: "",
        index: 15,
        group: 1
    },
    "nLSIZCh2DSI": {
        name: "Fever of unknown origin - DALY",
        data: [],
        val: 0,
        color: "",
        index: 0,
        group: 0
    },
    "YcSgwuKbLVM": {
        name: "Diarrhoeal diseases - DALY",
        data: [],
        val: 0,
        color: "",
        index: 1,
        group: 0
    },
    "Zy9AZUbeJdu": {
        name: "Cervix & uterus cancers - DALY",
        data: [],
        val: 0,
        color: "",
        index: 16,
        group: 1
    },
    "cmCvW0dqYIU": {
        name: "Cerebrovascular diseases - DALY",
        data: [],
        val: 0,
        color: "",
        index: 17,
        group: 1
    },
    "QrzJPtwAXVm": {
        name: "Venomous deaths - DALY",
        data: [],
        val: 0,
        color: "",
        index: 36,
        group: 2
    },
    "tFs20dNflp1": {
        name: "Heart diseases - DALY",
        data: [],
        val: 0,
        color: "",
        index: 18,
        group: 1
    },
    "YxmshxPQfFT": {
        name: "Drowning - DALY",
        data: [],
        val: 0,
        color: "",
        index: 37,
        group: 2
    },
    "QRUZa2NHY35": {
        name: "Skin, sense organs and Oral conditions - DALY",
        data: [],
        val: 0,
        color: "",
        index: 19,
        group: 1
    },
    "WK6PFCzY5ey": {
        name: "Other neuropsychiatric conditions (excl. epilepsy) - DALY",
        data: [],
        val: 0,
        color: "",
        index: 20,
        group: 1
    },
    "FtDFPXEUzVS": {
        name: "Hepatitis - DALY",
        data: [],
        val: 0,
        color: "",
        index: 2,
        group: 0
    },
    "cbvHNRyXPLX": {
        name: "Other infectious and parasitic diseases - DALY",
        data: [],
        val: 0,
        color: "",
        index: 3,
        group: 0
    },
    "x0bg1YZkRL7": {
        name: "Other digestive diseases - DALY",
        data: [],
        val: 0,
        color: "",
        index: 21,
        group: 1
    },
    "pf9fXcVMch1": {
        name: "Leukemia,lymphoma and oth. hemotopoietic malignancies - DALY",
        data: [],
        val: 0,
        color: "",
        index: 22,
        group: 1
    },
    "lXK2YGC8Wnm": {
        name: "Chronic respiratory diseases - DALY",
        data: [],
        val: 0,
        color: "",
        index: 23,
        group: 1
    },
    "E4ThmXrKE83": {
        name: "Meningitis/encephalitis - DALY",
        data: [],
        val: 0,
        color: "",
        index: 4,
        group: 0
    },
    "xtKp6I7IW26": {
        name: "All other injuries - DALY",
        data: [],
        val: 0,
        color: "",
        index: 38,
        group: 2
    },
    "sBzkGMgeqz5": {
        name: "Stomach cancer - DALY",
        data: [],
        val: 0,
        color: "",
        index: 24,
        group: 1
    },
    "Ken3MHrMEUR": {
        name: "Nephritis and nephrosis - DALY",
        data: [],
        val: 0,
        color: "",
        index: 25,
        group: 1
    },
    "xJTNZfIyiF7": {
        name: "Road traffic accidents - DALY",
        data: [],
        val: 0,
        color: "",
        index: 39,
        group: 2
    },
    "OaP6C0RHURO": {
        name: "Liver cancer - DALY",
        data: [],
        val: 0,
        color: "",
        index: 26,
        group: 1
    },
    "g57Aq6ENncW": {
        name: "Nutritional deficiencies - DALY",
        data: [],
        val: 0,
        color: "",
        index: 5,
        group: 0
    },
    "ed41dKPKdhx": {
        name: "Maternal conditions - DALY",
        data: [],
        val: 0,
        color: "",
        index: 12,
        group: 0
    },
    "p1eQckB6I12": {
        name: "Musculoskeletal diˀ DALY",
        data: [],
        val: 0,
        color: "",
        index: 27,
        group: 1
    },
    "dFv9IGNFyq5": {
        name: "Selected vaccine preventable diseases - DALY",
        data: [],
        val: 0,
        color: "",
        index: 6,
        group: 0
    },
    "s3E5MECaJga": {
        name: "Rheumatic heart disease - DALY",
        data: [],
        val: 0,
        color: "",
        index: 28,
        group: 1
    },
    "GaYrrMmz1KB": {
        name: "Lung & airway cancers - DALY",
        data: [],
        val: 0,
        color: "",
        index: 29,
        group: 1
    },
    "x6OFcxMCTVw": {
        name: "Respiratory infections - DALY",
        data: [],
        val: 0,
        color: "",
        index: 7,
        group: 0
    },
    "VuEOD18STGS": {
        name: "Perinatal conditions - DALY",
        data: [],
        val: 0,
        color: "",
        index: 8,
        group: 0
    },
    "ipm4P5vdH3O": {
        name: "Congenital anomalies - DALY",
        data: [],
        val: 0,
        color: "",
        index: 40,
        group: 2
    },
    "gxhQpCbFB3Q": {
        name: "All other cancers - DALY",
        data: [],
        val: 0,
        color: "",
        index: 30,
        group: 1
    },
    "mHrDw0dGvc9": {
        name: "Other digestive cancers - DALY",
        data: [],
        val: 0,
        color: "",
        index: 31,
        group: 1
    },
    "wPKc8aPWoI2": {
        name: "Tuberculosis DALY",
        data: [],
        val: 0,
        color: "",
        index: 9,
        group: 0
    },
    "sXDbduD9Sei": {
        name: "Oral and upper aerodigestive cancers - DALY",
        data: [],
        val: 0,
        color: "",
        index: 32,
        group: 1
    },
    "uqUmzU6bjbF": {
        name: "Malaria - DALY",
        data: [],
        val: 0,
        color: "",
        index: 10,
        group: 0
    },
    "yIyNAOM1u9Z": {
        name: "Breast cancer - DALY",
        data: [],
        val: 0,
        color: "",
        index: 33,
        group: 1
    },
    "CCPvTbThNQC": {
        name: "Sexually-transmitted infections incl. HIV/AIDS - DALY",
        data: [],
        val: 0,
        color: "",
        index: 11,
        group: 0
    },
    "OaeHix9duCn": {
        name: "Self-inflicted injuries (suicide) - DALY",
        data: [],
        val: 0,
        color: "",
        index: 41,
        group: 2
    },
    "FyUxNHcmGQb": {
        name: "Interpersonal violence - DALY",
        data: [],
        val: 0,
        color: "",
        index: 42,
        group: 2
    },
    "MgPog0EwzQi": {
        name: "Ill-defined or cause unknown - DALY",
        data: [],
        val: 0,
        color: "",
        index: 44,
        group: 3
    },
    "vmoQPXY2pVk": {
        name: "Geneto-urintary diseases - DALY",
        data: [],
        val: 0,
        color: "",
        index: 34,
        group: 1
    },
    "A5iw1YRUUoG": {
        name: "Gastro-oesophageal diseases - DALY",
        data: [],
        val: 0,
        color: "",
        index: 35,
        group: 1
    },
    "hz3xiC028bl": {
        name: "Falls - DALY",
        data: [],
        val: 0,
        color: "",
        index: 43,
        group: 2
    },

}];

const indiaOuId = "cBx1Gyq06xU";

const allouIDs = "J1qTQ3TKlNU;ft1FqZHkEIV;UBKSsG72Cg8;nCTGAP1pUqK;z8g8gAEeird;xHmXn2gn3rS;x2yMjFqOh5R;AOxuENOsJFH;YE9KMXV2qC3;dBKf3LDpfYl;N40kwU7hERO;kBajdL7waFu;XJMaF0ZzGo1;wTQrXc3kOgA;y85LzeVqQNo;YkAq1G0JN6Y;qA92TbABPdT;TABbglhSurr;B8ffozIh5D0;BBnVIaukxaN;x5TafwbmlFt;KfUK7GlZqiu;XX7mkLtUkZT;A64ajbtzogd;ITt5l6I85Uf;n5C3pZ1QbDV;XzbGlw8pVXQ;Cbw7y8SiYyW;VTNC2rB3AzO;nAEXiUnvksD;WwBDw1qLQfH;yWmW9qcMVwZ;yBFfg3amqn7;BvnPG4cWtyx;DrcfNamsJRj;N2K15gP6SKn;osxywsmbdA9;JQq12xOtj1a";

const statesMapData = [{
    // 'J1qTQ3TKlNU': {data : 0},
    'ft1FqZHkEIV': {
        data: 0
    },
    'UBKSsG72Cg8': {
        data: 0
    },
    'nCTGAP1pUqK': {
        data: 0
    },
    'z8g8gAEeird': {
        data: 0
    },
    // 'xHmXn2gn3rS': {data : 0},
    'x2yMjFqOh5R': {
        data: 0
    },
    'AOxuENOsJFH': {
        data: 0
    },
    // 'YE9KMXV2qC3': {data : 0},
    'dBKf3LDpfYl': {
        data: 0
    },
    'N40kwU7hERO': {
        data: 0
    },
    'kBajdL7waFu': {
        data: 0
    },
    'XJMaF0ZzGo1': {
        data: 0
    },
    'wTQrXc3kOgA': {
        data: 0
    },
    'y85LzeVqQNo': {
        data: 0
    },
    'YkAq1G0JN6Y': {
        data: 0
    },
    'qA92TbABPdT': {
        data: 0
    },
    'TABbglhSurr': {
        data: 0
    },
    // 'B8ffozIh5D0': {data : 0},
    'BBnVIaukxaN': {
        data: 0
    },
    'x5TafwbmlFt': {
        data: 0
    },
    'KfUK7GlZqiu': {
        data: 0
    },
    'XX7mkLtUkZT': {
        data: 0
    },
    'A64ajbtzogd': {
        data: 0
    },
    'ITt5l6I85Uf': {
        data: 0
    },
    'XzbGlw8pVXQ': {
        data: 0
    },
    'VTNC2rB3AzO': {
        data: 0
    },
    'nAEXiUnvksD': {
        data: 0
    },
    'WwBDw1qLQfH': {
        data: 0
    },
    'yWmW9qcMVwZ': {
        data: 0
    },
    'yBFfg3amqn7': {
        data: 0
    },
    // 'BvnPG4cWtyx': {data : 0},
    'DrcfNamsJRj': {
        data: 0
    },
    'N2K15gP6SKn': {
        data: 0
    },
    'osxywsmbdA9': {
        data: 0
    },
    'JQq12xOtj1a': {
        data: 0
    },
    'Cbw7y8SiYyW': {
        data: 0
    }
}];


const statesMapName = [{
    // 'J1qTQ3TKlNU': 'in-', andaman nicobar
    'ft1FqZHkEIV': {
        name: 'in-ap'
    },
    'UBKSsG72Cg8': {
        name: 'in-ar'
    },
    'nCTGAP1pUqK': {
        name: 'in-as'
    },
    'z8g8gAEeird': {
        name: 'in-br'
    },
    // 'xHmXn2gn3rS': 'in-', chandigarh
    'x2yMjFqOh5R': {
        name: 'in-ct'
    },
    'AOxuENOsJFH': {
        name: 'in-dn'
    },
    // 'YE9KMXV2qC3': 'in-',daman} and diu
    'dBKf3LDpfYl': {
        name: 'in-dl'
    },
    'N40kwU7hERO': {
        name: 'in-ga'
    },
    'kBajdL7waFu': {
        name: 'in-2984'
    },
    'XJMaF0ZzGo1': {
        name: 'in-hr'
    },
    'wTQrXc3kOgA': {
        name: 'in-hp'
    },
    'y85LzeVqQNo': {
        name: 'in-jk'
    },
    'YkAq1G0JN6Y': {
        name: 'in-jh'
    },
    'qA92TbABPdT': {
        name: 'in-ka'
    },
    'TABbglhSurr': {
        name: 'in-kl'
    },
    // 'B8ffozIh5D0': 'in-', Lakshadweep
    'BBnVIaukxaN': {
        name: 'in-mp'
    },
    'x5TafwbmlFt': {
        name: 'in-mh'
    },
    'KfUK7GlZqiu': {
        name: 'in-mn'
    },
    'XX7mkLtUkZT': {
        name: 'in-ml'
    },
    'A64ajbtzogd': {
        name: 'in-mz'
    },
    'ITt5l6I85Uf': {
        name: 'in-nl'
    },
    'XzbGlw8pVXQ': {
        name: 'in-or'
    },
    'VTNC2rB3AzO': {
        name: 'in-py'
    },
    'nAEXiUnvksD': {
        name: 'in-pb'
    },
    'WwBDw1qLQfH': {
        name: 'in-rj'
    },
    'yWmW9qcMVwZ': {
        name: 'in-sk'
    },
    'yBFfg3amqn7': {
        name: 'in-tn'
    },
    // 'BvnPG4cWtyx': 'in-', telangana
    'DrcfNamsJRj': {
        name: 'in-tr'
    },
    'N2K15gP6SKn': {
        name: 'in-up'
    },
    'osxywsmbdA9': {
        name: 'in-ut'
    },
    'JQq12xOtj1a': {
        name: 'in-wb'
    }
}];

const stateNames = [{
    "J1qTQ3TKlNU": "Andaman and Nicobar",
    "ft1FqZHkEIV": "Andhra Pradesh",
    "UBKSsG72Cg8": "Arunachal Pradesh",
    "nCTGAP1pUqK": "Assam",
    "z8g8gAEeird": "Bihar",
    "xHmXn2gn3rS": "Chandigarh",
    "x2yMjFqOh5R": "Chhattisgarh",
    "AOxuENOsJFH": "Dadra and Nagar Haveli",
    "YE9KMXV2qC3": "Daman and Diu",
    "dBKf3LDpfYl": "Delhi",
    "N40kwU7hERO": "Goa",
    "kBajdL7waFu": "Gujarat",
    "XJMaF0ZzGo1": "Haryana",
    "wTQrXc3kOgA": "Himachal Pradesh",
    "y85LzeVqQNo": "Jammu and Kashmir",
    "YkAq1G0JN6Y": "Jharkhand",
    "qA92TbABPdT": "Karnataka",
    "TABbglhSurr": "Kerala",
    "B8ffozIh5D0": "Lakshadweep",
    "BBnVIaukxaN": "Madhya Pradesh",
    "x5TafwbmlFt": "Maharashtra",
    "KfUK7GlZqiu": "Manipur",
    "XX7mkLtUkZT": "Meghalaya",
    "A64ajbtzogd": "Mizoram",
    "ITt5l6I85Uf": "Nagaland",
    "n5C3pZ1QbDV": "Northeastern cluster States",
    "XzbGlw8pVXQ": "Odisha",
    "Cbw7y8SiYyW": "Other remaining small states and UTs",
    "VTNC2rB3AzO": "Puducherry",
    "nAEXiUnvksD": "Punjab",
    "WwBDw1qLQfH": "Rajasthan",
    "yWmW9qcMVwZ": "Sikkim",
    "yBFfg3amqn7": "Tamil Nadu",
    "BvnPG4cWtyx": "Telangana",
    "DrcfNamsJRj": "Tripura",
    "N2K15gP6SKn": "Uttar Pradesh",
    "osxywsmbdA9": "Uttarakhand",
    "JQq12xOtj1a": "West Bengal",
}];

const stateNamesAndId = [{
    "AndamanandNicobar":"J1qTQ3TKlNU",
    "AndhraPradesh":"ft1FqZHkEIV",
    "ArunachalPradesh":"UBKSsG72Cg8",
    "Assam":"nCTGAP1pUqK",
    "Bihar":"z8g8gAEeird",
    "Chandigarh":"xHmXn2gn3rS",
    "Chhattisgarh":"x2yMjFqOh5R",
    "DadraandNagarHaveli":"AOxuENOsJFH",
    "DamanandDiu":"YE9KMXV2qC3",
    "Delhi":"dBKf3LDpfYl",
    "Goa":"N40kwU7hERO",
    "Gujarat":"kBajdL7waFu",
    "Haryana":"XJMaF0ZzGo1",
    "HimachalPradesh":"wTQrXc3kOgA",
    "JammuandKashmir":"y85LzeVqQNo",
    "Jharkhand":"YkAq1G0JN6Y",
    "Karnataka":"qA92TbABPdT",
    "Kerala":    "TABbglhSurr",
    "Lakshadweep":"B8ffozIh5D0",
    "MadhyaPradesh":    "BBnVIaukxaN",
    "Maharashtra":"x5TafwbmlFt",
    "Manipur":"KfUK7GlZqiu",
    "Meghalaya":"XX7mkLtUkZT",
    "Mizoram":"A64ajbtzogd",
    "Nagaland":"ITt5l6I85Uf",
    "NortheasternclusterStates":"n5C3pZ1QbDV",
    "Odisha":"XzbGlw8pVXQ",
    "OtherremainingsmallstatesandUTs":"Cbw7y8SiYyW",
    "Puducherry":"VTNC2rB3AzO",
    "Punjab":    "nAEXiUnvksD",
    "Rajasthan":"WwBDw1qLQfH",
    "Sikkim":"yWmW9qcMVwZ",
    "TamilNadu":"yBFfg3amqn7",
    "Telangana":"BvnPG4cWtyx",
    "Tripura":    "DrcfNamsJRj",
    "UttarPradesh":"N2K15gP6SKn",
    "Uttarakhand":"osxywsmbdA9",
    "WestBengal":"JQq12xOtj1a"
}];

export default {
    yll: yll_id,
    yld: yld_id,
    daly: daly_id,
    deaths: death_id,
    population: death_id,
    age_id: age_id,
    age_0_4_id: age_0_4_id,
    age_5_14_id: age_5_14_id,
    age_15_29_id: age_15_29_id,
    age_30_49_id: age_30_49_id,
    age_50_59_id: age_50_59_id,
    age_60_69_id: age_60_69_id,
    age_70_79_id: age_70_79_id,
    age_above80_id: age_above80_id,
    all_ages_main_id: all_ages_main_id,
    all_ages_id: all_ages_id,
    gender_id: gender_id,
    gender_male_id: gender_male_id,
    gender_female_id: gender_female_id,
    site_id: site_id,
    site_urban_id: site_urban_id,
    site_rural_id: site_rural_id,
    age_main_var: age_main_var,
    gender_main_var: gender_main_var,
    diseases_yll: diseases_obj_yll,
    ages_all: ages_obj,
    diseases_daly: diseases_obj_daly,
    diseases_yld: diseases_obj_yld,
    diseases_group: diseases_group,
    diseases_group_1: diseases_group_1,
    diseases_group_2: diseases_group_2,
    diseases_group_3: diseases_group_3,
    diseases_group_4: diseases_group_4,

    diseases_deaths: diseases_obj_deaths,
    age_categories: age_categories,
    gender_categories: gender_categories,
    gender_categories_forpie: gender_categories_forpie,
    site_main_var: site_main_var,
    site_categories: site_categories,
    site_categories_forpie: site_categories_forpie,
    indiaOuId: indiaOuId,
    statesMapData: statesMapData,
    allouIDs: allouIDs,
    statesMapName: statesMapName,
    stateNames: stateNames,
    stateNamesAndId: stateNamesAndId
}